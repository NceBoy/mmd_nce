...
class VFNetMultiAnchor(ATSSHead, FCOSHead):
    ...
    def __init__(self,
        ...
        self.reg_channel = len(anchor_generator["ratios"])
        self.anchor_generator = build_anchor_generator(anchor_generator)
        self.num_anchors = self.anchor_generator.num_base_anchors[0]
        self.reg_cls_branch = reg_cls_branch
        self.objectness = objectness
        if depth_size:
            self.groups = self.feat_channels
        else:
            self.groups = 1
        ...
        if denorm:
            ...
            # self.norm_factor = [1 for regress_range in regress_ranges]
        else:
            self.reg_denoms = [1 for regress_range in regress_ranges]
            # self.norm_factor = self.strides
    ...
    def _init_layers(self):
        ...
        if self.objectness:
            self.mlvl_objectness = nn.ModuleList()
        ...
        for level in range(self.num_out):
            ...
            vfl_cls_convs = nn.Conv2d(self.feat_channels, self.cls_out_channels*self.reg_channel, 1, padding=0)
            vfl_reg_convs = nn.Conv2d(self.feat_channels, self.feat_channels, 3,
                                      groups=self.groups, stride=1, padding=1)

            vfl_refine_convs = nn.Conv2d(self.feat_channels, 4*self.reg_channel, 1, padding=0)
            vfl_reg = nn.Conv2d(self.feat_channels, 4*self.reg_channel, 1, padding=0)
            vfl_reg_cls = nn.Conv2d(self.feat_channels, self.cls_out_channels, 1, padding=0)

            if self.objectness:
                objectness = nn.Conv2d(self.feat_channels, self.cls_out_channels*self.reg_channel, 1, padding=0)
                self.mlvl_objectness.append(objectness)
    ...
    def init_weights(self):
        ...
        for level in range(self.num_out):
            ...
            normal_init(self.mlvl_vfl_reg_convs[level], std=0.01)
            normal_init(self.mlvl_vfl_refine_convs[level], std=0.01)
            normal_init(self.mlvl_objectness[level], std=0.01, bias=1)
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom, level, onnx=False):
        ...
        if self.objectness:
            objectness = self.mlvl_objectness[level](reg_feat)
        ...
        if self.reg_cls_branch:
            cls_score = self.mlvl_vfl_reg_cls[level](cls_feat)
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            pos_points = flatten_points.repeat(1, self.reg_channel).reshape(-1, 2)[pos_inds]
            # pos_points = flatten_points.reshape(-1, 2)[pos_inds]

            # pos_points = flatten_points[pos_inds]
            # pos_weights = flatten_cls_scores.detach().sigmoid()
            # pos_weights = pos_weights.max(dim=1)[0][pos_inds]
            ...
            # for i in range(self.reg_channel):
            #     pos_decoded_bbox_pred = distance2bbox(pos_points[i::self.reg_channel], pos_bbox_preds[i::self.reg_channel])
            #     pos_decoded_target_pred = distance2bbox(pos_points[i::self.reg_channel], pos_bbox_targets[i::self.reg_channel])
            #     pos_decoded_bbox_preds.append(pos_decoded_bbox_pred)
            #     pos_decoded_target_preds.append(pos_decoded_target_pred)
            # pos_decoded_bbox_preds = torch.cat(pos_decoded_bbox_preds)
            # pos_decoded_target_preds = torch.cat(pos_decoded_target_preds)
            ...
            # 如果使用bbox_weights_ini作为bbox_weights，那么很容易出现一开始Bbox loss非常低的情况(iou趋近于0)，那么CIOU
            # 无法起到作用。
            loss_bbox = self.loss_bbox(
                pos_decoded_bbox_preds,
                pos_decoded_target_preds.detach(),
            ...
            if self.use_vfl:
                # flatten_cls_scores = flatten_cls_scores.repeat(1, self.reg_channel).reshape(-1, 1)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            if self.denorm:
                ...
                bbox_pred_list = [
                    bbox_preds_refine[i][img_id].detach() * self.strides[i] * 6
                    for i in range(num_levels)
                ]
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, points in zip(cls_scores, bbox_preds,
            ...
            points = points.repeat(1, self.reg_channel).view(-1, 2)
    ...
    def get_targets_bbox_denorm(self,
                    anchor_list,
                    valid_flag_list,
                    gt_bboxes_list,
                    img_metas,
        ...
        for i in range(num_imgs):
            ...
            anchor_list[i] = torch.cat(anchor_list[i])              #anchor_list.shape = [num_img, num_total_anchor, 4]
            valid_flag_list[i] = torch.cat(valid_flag_list[i])      #valid_flag_list.shape = [num_img, num_total_anchor, 4]
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_target_single_bbox_denorm,
             anchor_list,
             valid_flag_list,
    ...
    def _get_target_single_bbox_denorm(self,
                           flat_anchors,
                           valid_flags,
                           num_level_anchors,
                           gt_bboxes,
        ...
        inside_flags = anchor_inside_flags(flat_anchors, valid_flags,
                                           ...
                                           self.train_cfg.allowed_border) #筛选出image内的anchor
        ...
        assign_result = self.assigner.assign(anchors, num_level_anchors_inside,
                                             ...
                                             gt_labels) #筛选出每个尺度 inside achor的数量
        ...
        if unmap_outputs:
            ...
            anchors = unmap(anchors, num_total_anchors, inside_flags)               #把所有有效的anchor装到total_num_anchors数量的tensor中
            labels = unmap(
            ...
            # start = 0
            # for level in range(len(num_level_anchors)):
            #     end = start + num_level_anchors[level]
            #     ctx = (bbox_targets[start:end, 0] + bbox_targets[start:end, 2]) / 2
            #     cty = (bbox_targets[start:end, 1] + bbox_targets[start:end, 3]) / 2
            #     w = bbox_targets[start:end, 2] - bbox_targets[start:end, 0]
            #     h = bbox_targets[start:end, 3] - bbox_targets[start:end, 1]
            #     bbox_targets[start:end, 0] = ctx - w / 2
            #     bbox_targets[start:end, 1] = cty - h / 2
            #     bbox_targets[start:end, 2] = ctx + w / 2
            #     bbox_targets[start:end, 3] = cty + h / 2
            #     # bbox_targets[start:end] = bbox_targets[start:end] / self.strides[level]
            #     start = end
    ...
    def get_atss_targets(self,
        ...
        if self.denorm:
            ...
            cls_reg_targets = self.get_targets_bbox_denorm(
                anchor_list,
                valid_flag_list,
                gt_bboxes,
                img_metas,
        ...
        mlvl_points = [points.repeat(1, self.reg_channel).view(-1, 2) for points in mlvl_points]
    ...
    def transform_bbox_targets(self, decoded_bboxes, mlvl_points, num_imgs):
        ...
        for i in range(num_levels):
            if self.denorm:
                ...
                bbox_target = bbox2distance(mlvl_points[i], decoded_bboxes[i]) / self.strides[i] / 6
